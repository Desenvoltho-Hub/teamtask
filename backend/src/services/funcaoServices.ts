...
export const funcaoCreate = async ({
  task,
  title,
  user,
}: {
  ...
  if (!task || !title || !user) {
    throw new Error(
      "Id da task, titulo e user são necessários para criar uma função!"
    );
  }
...
export const removerFunc = async (id: string) => {
  if (!id) {
    throw new Error("Não foi possível achar a função");
  }
  const response = await Funcao.findByIdAndDelete(id);
  return response;
};
